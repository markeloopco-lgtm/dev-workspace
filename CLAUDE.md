...
- [ ] ショート1本目を通しで作る（docs/07。立ち絵PNG＋SBV2のWAVがあれば配信システム抜きで完結。
      **ユーザーの現在の関心はショート量産**。参考動画は「人気声優・◯◯さん(39)、
      ...
      ユーザーの意向でこの型を再現する方針になり、make_short.py に matome スタイル
      （見出しバナー＋写真スローズーム＋反応カード）を実装済み。
      `init --style matome` でテンプレが出る。画像の権利はユーザー責任である旨を
      docs/07 に明記済み。実在人物を性的に扱うテーマ選定は断った。映像そのものは未確認）
