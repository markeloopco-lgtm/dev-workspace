...
- `scripts/auto_edit.py`: 録画の自動編集（ジェットカット+テロップ+BGM）。設定は `configs/auto_edit.yaml`。テロップ様式は preset で切替（business=ビジネス系YouTube・既定 / talk=対談 / news=報道番組風）。各presetの数値は実在チャンネルの調査に較正済み（根拠と出典はdocs/06末尾）
