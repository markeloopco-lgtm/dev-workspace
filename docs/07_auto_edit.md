...
| preset | 見た目 | 参考にした型 |
|---|---|---|
| **talk**（既定） | 座布団なし。**話者ごとに文字色を変える**。カットは最も詰める | 年収チャンネルなどの対談 |
...
```yaml
style:
  font_size_ratio: 0.105          # 1行のとき (1080pで約113px)
  font_size_ratio_wrapped: 0.085  # 2行のとき (同 約92px)
```

同じ数値でも**フォントによって見た目の大きさが変わる**（Dela Gothic One や
UDゴシックは字面が大きい）。書体を決めてから大きさを詰めるのが早い。

### 「大きく」と「横幅いっぱい」は両立しない

長い文を1行に収めようとすると、画面幅に入るまで文字が自動で縮む。
つまり **1行あたりの文字数を減らすほど、文字は大きくできる**。

| やりたいこと | `max_chars_per_line` |
|---|---|
| 文字を大きく見せたい | **13〜16 の固定値** |
| 横幅を余さず使いたい | `auto`（長い文は縮んで1行に収まる） |

talk preset は「文字を大きく」を優先して **15文字固定**にしてある。

### 2行にせず、常に1行で出す

`telop.max_lines: 1` にすると折り返さず、長い発言は**短いテロップに分けて
順に表示**される（切り抜き動画でよくある見せ方）。talk preset はこの設定。

1枚ずつ独立して出るので、**分ける位置**が重要になる。句読点を最優先に、
無ければ「助詞の直後かつ次が漢字・カタカナ」の位置で切る。
...
「年収で／したか」のように語の途中で切れないようにしてある。
2行にしたい場合は `max_lines: 2` に戻すと、行長を揃えた折り返しになる。

## 1行に入れる文字数（横幅の使い方）

`telop.max_chars_per_line` を **`auto`** にすると、画面幅と文字サイズから
**入るだけ入れる**（＝横幅を使い切る）。数値を書けばその文字数で固定される。

`auto` の判定は「2行にしたときの文字サイズまでなら1行で通す」。少しはみ出す
だけの文は文字がわずかに縮んで1行に収まり、それより長いものだけ2行になる。
1行を縮めても2行のときより小さくならないので、読みにくくはならない。

| preset | 1行の上限（1920x1080） |
|---|---|
| talk | 15文字（文字を大きく見せるため固定） |
| business | `auto` = 約22文字 |
| news | 15文字（放送基準に合わせて固定） |

左右の余白は `style.safe_margin_ratio`（既定 0.04 = 片側4%）。
もっと端まで使いたければ 0.02 くらいまで下げられる。
...
| 要素 | 設定 | 根拠 |
|---|---|---|
| **出入りのフェード** | `telop.fade_in` / `fade_out`（既定 0.08秒） | 瞬時に出ると素人っぽくなる。放送のテロップは一拍かけて現れる |
| **表示時間の余裕** | `telop.duration_padding`（既定 0.5秒） | 「読み終わる時間＋0.5秒」が目安 |
| **サイドテロップ** | `side.text`（改行で2行） | テレビで定番の画面端の常時表示。右上が基本で、番組の印象を決める |

サイドテロップは「1行目＝色付きの小見出し」「2行目＝本題」の2段構成にしている。

```yaml
side:
  text: "特集\nエンジニアの転職"   # 空なら非表示
  position: top-right             # 放送では右上が定番
```

出しっぱなしにするものなので、**画面上10分の1程度**に収まる大きさにしてある。

なお、テロップは**出しすぎない**のが基本（見せたい場面で必要なものだけ）。
全発言を出すか要点だけにするかは、`telop.srt` から不要な行を消して調整する。
...
| 1行が短くて余白が目立つ | `max_chars_per_line: auto` にする / `safe_margin_ratio` を下げる |
| 帯が濃い/薄い | `band.opacity` (news preset。放送の目安は0.70〜0.85) |
