...
| **話者の区別** | 話者ごとに色を変える（`speakers:`）。座布団なしなら**縁取りの色**、`band.enabled: true` にすると**座布団の色** | 「イメージカラーから誰の発言か分かるように」 |
...
```yaml
style:
  font_size_ratio: 0.090          # 1行のとき (1080pで約97px)
  font_size_ratio_wrapped: 0.070  # 2行のとき (同 約76px)
```
...
座布団なし（既定）ならこの色が**縁取り**に、`band.enabled: true` なら**座布団**に使われる。
