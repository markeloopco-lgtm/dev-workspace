...
- 描画結果は `renders\cache\<16桁の英数字>\` に保存され、**同じ場面・同じ長さ**なら2回目から再利用される。
  台詞を直すとその後ろの宇宙ショットは長さや並びが変わるので描き直しになる。おすすめの進め方:
  1. `--engine 2d`（または `--draft`）で台本と尺を固める
  2. `renders\xxx.plan.json` の `speeds` の値を、台本の voices の `speed:` に数値で書き写して固定する
  3. 最後に1回だけBlenderで本番を書き出す
- キャッシュは1フレーム約0.1〜0.2MB（10分動画で1〜2GB程度）。30日使っていないものと、合計30GBを超えた分は
  書き出しのたびに自動で消える。すぐ消したい時は `vlab clean-cache --all`（音声のキャッシュ `renders\cache\tts\` は残る）
- 最初は `--draft`（半分の解像度）で1本通して、所要時間を確かめる
- 書き出し中はWindowsの自動スリープを止めている。ただし**ノートPCのふたは閉じない**（ふたを閉じた時の動作は止められない）
- 重い場合: `episodes` の宇宙シーンを減らす／`--engine 2d` に戻す
- Blenderが途中で失敗した場合、その種類の宇宙シーンは自動で2D版に切り替わり、最後に何ショット切り替えたか表示される
- 惑星の表面(大陸やクレーター)は同じプリセットなら毎回同じ。背景の星空は素材ごとに変わる
